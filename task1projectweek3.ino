...
// ================= TIME SETTINGS =================
#define MIN_RUN_TIME 16500UL  // 16.5 seconds minimum before allowed to stop
...
void loop() {
  ...
  // Distance
  float avgDistance = getAverageDistanceCm();  // in cm

  // ========= NEW: 1.8 m (180 cm) PAUSE LOGIC =========
  if (isMoving && !pauseDone && avgDistance >= 180.0) {  // 1.8 m
    pauseDone = true;        // only once

    stopMotors();
    unsigned long pauseStart = millis();

    // 3 second pause, keep updating LCD
    while (millis() - pauseStart < 3000) {
      unsigned long e  = millis() - start_time;
      int seconds      = e / 1000;
      int milliseconds = e % 1000;
      float dNow       = getAverageDistanceCm();
    ...
    // After 3 seconds, continue moving
    moveForward();
  }
